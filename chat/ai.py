...
- **`wallet_volume_rollup`** (~1.8M rows, per-wallet lifetime). Columns: `wallet`, `maker_volume_usd`, `taker_volume_usd`, `total_volume_usd`, `buy_volume_usd`, `sell_volume_usd`, matching trade counts, `total_fees_paid_usd`, `total_redemption_usd`, `redemption_count`, `last_redemption_at`, `first_active`, `last_active`, `active_months`, `markets_touched`. **Use for "big vs small wallets", "top traders", wallet rankings, wallet classification by any volume axis, redemption activity.** Indexes on `total_volume_usd DESC`, `total_redemption_usd DESC`, `last_active`, `maker_volume_usd`, `taker_volume_usd`. **PnL is NOT computable from this table.** The naive `sell_volume_usd − buy_volume_usd + total_redemption_usd − total_fees_paid_usd` undercounts costs for any wallet that uses CTF `PositionSplit` (USDC → YES+NO) or `PositionsMerge` (YES+NO → USDC), which the indexer doesn't track. Market makers (the largest wallets) make virtually all their inventory via splits, so this formula reads their split-and-sell-spread strategy as +$B PnL. If a user asks for "who made/lost money", explain that PnL requires split/merge data we don't yet index; offer instead "top wallets by redemption payout" (`ORDER BY total_redemption_usd DESC`) or "wallets with highest sell-vs-buy cash ratio in markets they participate in" — both are honest proxies.
